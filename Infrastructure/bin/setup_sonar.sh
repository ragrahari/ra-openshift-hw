...
oc set probe dc/sonarqube --liveness --failure-threshold=3 --initial-delay-seconds=40 -- echo ok --period-seconds=10 --success-threshold=1 --timeout-seconds=360 -n ${GUID}-sonarqube
oc set probe dc/sonarqube --readiness --failure-threshold=3 --initial-delay-seconds=40 --get-url=http://:9000/about --period-seconds=10 --success-threshold=1 --timeout-seconds=360 -n ${GUID}-sonarqube
# Expose the service
