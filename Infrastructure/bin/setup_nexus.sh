...
oc set probe deploymentconfig/nexus3 --liveness --failure-threshold=3 --initial-delay-seconds=60 -- echo ok --period-seconds=10 --success-threshold=1 --timeout-seconds=420 -n ${GUID}-nexus
oc set probe deploymentconfig/nexus3 --readiness --failure-threshold=3 --initial-delay-seconds=60 --get-url=http://:8081/repository/maven-public/ --period-seconds=10 --success-threshold=1 --timeout-seconds=420 -n ${GUID}-nexus
